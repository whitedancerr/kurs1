...
export function Login() {
  ...
  const from =
    state?.from && typeof state.from === 'string'
      ? state.from
      : '/profile';
      ...
      <div className="max-w-md w-full">
        <div className="bg-card text-card-foreground rounded-2xl shadow-xl border border-border p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            {justRegistered && (
              <div className="bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-200 px-4 py-3 rounded-lg text-sm">
                Регистрация прошла успешно. Если включено подтверждение email, перейдите по ссылке из письма, затем войдите.
              </div>
            )}
            {loginError && (
